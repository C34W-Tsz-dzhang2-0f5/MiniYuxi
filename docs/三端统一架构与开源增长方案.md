...
|---|---|---|
| 会话常驻/恢复/重连 | 单进程内 | sidecar 常驻 + `SessionStore` 重建（s07） |
| 工具上下文爆炸 | toolset 过滤已有 | 加 `tool_search` 渐进披露（s03） |
| 大输出塞不进 context | 无 | 工具结果写磁盘、context 留指针（s13） |
| 工具命令变后门 | `hardline_block` 已有 | 子进程环境隔离：不继承 provider 凭证/SSH agent（s23） |
| 六层解耦 | 已分层 | 桌面外壳只依赖 `localhost` 契约，不碰 `core` 内部 |
...
uv run python examples/desktop_tour/code.py
```
...
| **P3 桌面端** | Tauri sidecar | `apps/desktop/` + `examples/desktop_tour` | 🟡 中（sidecar 生命周期） |
...
| `README.md`（待升级） | 开源落地页 |
| `LICENSE` / `CONTRIBUTING.md` / `SECURITY.md` / `.github/`（待建） | 开源基建 |
| 既有 `core/`、`api.py`、`tests/`（20/20 + 13 + 5） | 已就绪的内核与质量凭证 |

> 下一步建议：先升级 `README.md` 为落地页 + 建 `.github` 卡点，再启动 `apps/cli`（最快见效），最后做 `apps/desktop`（sidecar 复用内核，风险可控）。
